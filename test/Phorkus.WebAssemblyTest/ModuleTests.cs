    ...
    public class ModuleTests
    {
        ...
        public void Module_ReadFromBinaryStream()
        {
            Assert.AreEqual("input",
                Assert.ThrowsException<ArgumentNullException>(() => Module.ReadFromBinary((Stream) null)).ParamName);
            ...
            using (var sample = new MemoryStream())
            {
                ...
                sample.Position = 0;
                Assert.IsTrue(Assert.ThrowsException<ModuleLoadException>(() => Module.ReadFromBinary(sample)).Message
                    .ToLowerInvariant().Contains("magic"));
                ...
                Assert.IsInstanceOfType(
                    Assert.ThrowsException<ModuleLoadException>(() => Module.ReadFromBinary(sample)).InnerException,
                ...
                sample.Position = 0;
                Assert.IsTrue(Assert.ThrowsException<ModuleLoadException>(() => Module.ReadFromBinary(sample)).Message
                    .ToLowerInvariant().Contains("version"));
        ...
        public void Module_InstructionSequenceMissingEndValidation()
        {
            Assert.ThrowsException<InvalidOperationException>(() =>
                new Module {Globals = new[] {new Global()}}.WriteToBinaryNoOutput());
            Assert.ThrowsException<InvalidOperationException>(() =>
                new Module {Elements = new[] {new Element()}}.WriteToBinaryNoOutput());
            Assert.ThrowsException<InvalidOperationException>(() =>
                new Module {Codes = new[] {new FunctionBody()}}.WriteToBinaryNoOutput());
            Assert.ThrowsException<InvalidOperationException>(() =>
                new Module {Data = new[] {new Data()}}.WriteToBinaryNoOutput());
        }
    }
}