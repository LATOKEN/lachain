...
namespace Phorkus.HermesTest
{
    ...
    public class DefaultTest
    {
        ...
        {
            var plainRnd = new JavaRandom(123456789);
            ...
            plainRnd = new JavaRandom(98428965);
            PublicParameters publicParameters = Util.generateParamsforBitcoin(60, 256, plainRnd, paillierKey);
            
            plainRnd = new JavaRandom(58984789);
            ...
            BigInteger randomness = Util.randomFromZnStar(BitcoinParams.Instance.q, plainRnd);
            BigInteger encryptedPrivateKey = new Paillier(paillierKey).encrypt(privateKey, randomness);
            ECPoint publicKey = BitcoinParams.Instance.G.Multiply(privateKey.Mod(BitcoinParams.Instance.q)).Normalize();
            ...
            foreach (DSASignature signature in signatures) {
                ...
                BigInteger invS = signature.s.ModInverse(BitcoinParams.Instance.q);
                ECPoint validation =
                    publicKey.Multiply(signature.r.Multiply(invS).Mod(BitcoinParams.Instance.q))
                        .Add(BitcoinParams.Instance.G.Multiply(mprime.Multiply(invS).Mod(BitcoinParams.Instance.q)));
                BigInteger vX = validation.Normalize().XCoord.ToBigInteger().Mod(BitcoinParams.Instance.q);
