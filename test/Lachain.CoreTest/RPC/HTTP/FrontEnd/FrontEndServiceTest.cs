...
namespace Lachain.CoreTest.RPC.HTTP.FrontEnd
{
    public class FrontEndServiceTest
    {
        ...
        public void Test_PasswordChange()
        {
            ...
            Assert.AreEqual("incorrect_password", _fes?.UnlockWallet("12345", 1000));
            
            // Unlock the wallet with correct password
            Assert.AreEqual("unlocked", _fes?.UnlockWallet("abcde", 1000));
        }
    }
}