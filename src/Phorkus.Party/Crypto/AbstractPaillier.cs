...
using Phorkus.Party.Crypto.Zkp;
 ...
 */
    public abstract class AbstractPaillier
    {
        ...
        {
            if (encryptMode == false) throw new ArgumentException(this.notReadyForEncryption);
        ...
        public static BigInteger encrypt(BigInteger m, BigInteger r, BigInteger n, BigInteger ns, BigInteger nSPlusOne)
        {
            ...
            {
                throw new ArgumentException("m must be less than n^s");
            }

            if (!(PaillierKey.inModNStar(r, n)))
            {
                throw new ArgumentException("r must be relatively prime to n and 0 <= r < n");
            }
        ...
        {
            if (encryptMode == false) throw new ArgumentException(this.notReadyForEncryption);
            if (!(key.inModNSPlusOne(c1))) throw new ArgumentException("c1 must be less than n^(s+1)");
            if (!(key.inModNSPlusOne(c2))) throw new ArgumentException("c2 must be less than n^(s+1)");
        ...
        {
            if (c1.Abs().CompareTo(nsplus1) >= 0) throw new ArgumentException("c1 must be less than n^(s+1)");
            if (c2.Abs().CompareTo(nsplus1) >= 0) throw new ArgumentException("c2 must be less than n^(s+1)");
        ...
        {
            if (encryptMode == false) throw new ArgumentException(this.notReadyForEncryption);
            if (!(key.inModNSPlusOne(c1))) throw new ArgumentException("c1 must be less than n^2");
        ...
        {
            if (c.Abs().CompareTo(nSquare) >= 0) throw new ArgumentException("c1 must be less than n^2");
        ...
        public BigInteger randomize(BigInteger c, BigInteger r)
        {
            ...
            if (!(key.inModNStar(r))) throw new ArgumentException("r must be relatively prime to n and 0<=r<n");
