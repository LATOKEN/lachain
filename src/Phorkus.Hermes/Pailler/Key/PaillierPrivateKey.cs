 ...
 * @see PaillierKey
 */
public class PaillierPrivateKey : PaillierKey {
	 ...
	 * This Serial ID
	 */
	private static long serialVersionUID = 5852433647485331139L;
	...
	:base(n, seed)
	{
		...
		if (!(inModNStar(d.Mod(n))))
			throw new ArgumentException("d must be relatively prime to n");
		this.d = d;
	...
	:base(p,q,seed){
		...
		if (!(d.Mod(lambda).CompareTo(BigInteger.Zero)==0))
			throw new ArgumentException("d must be a multiple of lcm(p-1,q-1)");
	...
	public byte[] toByteArray() {
		...
		byte[] p = base.toByteArray(); // previous layer
