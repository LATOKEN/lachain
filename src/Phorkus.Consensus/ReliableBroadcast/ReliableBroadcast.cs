...
namespace Phorkus.Consensus.ReliableBroadcast
{
    public class ReliableBroadcast : AbstractProtocol
    {
        ...
        private void HandleInputMessage(ProtocolRequest<ReliableBroadcastId, EncryptedShare> broadcastRequested)
        {
            ...
            {
                //_broadcaster.SendToValidator(CreateValMessage(input, indexAddressee), indexAddressee); // for debug
                _broadcaster.SendToValidator(CreateValMessage(correctRealInput, indexAddressee), indexAddressee); // realInput
        ...
        private UInt256 RecalculateMerkleRoot(List<int> goodBlocks)
        {
            ...
            {
                newHashes.Add(BitConverter.GetBytes(block).Keccak256().ToUInt256());
            }
        ...
        private void HandleEchoMessage(Validator messageValidator, ECHOMessage echo)
        {
            ...
            if (_countCorrectECHOMsg >= N - F)
            {
                ...
                if (receivedRoots.SequenceEqual(recalculatedRootsToSend))
                {
                    ...
                    {
                        var tmp = GetMyId();
                        _broadcaster.Broadcast(CreateReadyMessage(recalculatedRootsToSend, echo.Plaintext.ToArray()));
        ...
        private void HandleReadyMessage(Validator validator, ReadyMessage readyMessage)
        {
            ...
            if (CheckReadyMsg(rootsToCheck))
            {
                ...
                if (_countReadyMsg == F + 1)
                {
                    if (!_sentReadyMsg[GetMyId()])
                    {
                        _broadcaster.Broadcast(CreateReadyMessage(rootsToCheck, plaintext));
                        _sentReadyMsg[GetMyId()] = true;
                        Console.Error.WriteLine(
                            "Thread # {0} ID {1} _sentReadyMsg[{1}] = {2}", 
                            Thread.CurrentThread.ManagedThreadId, GetMyId(), _sentReadyMsg[GetMyId()]);
        ...
        private  Tuple<bool, List<int>, List<UInt256>> CheckEchoMsgNew(ECHOMessage msg)
        {
            ...
            foreach (var currentRegion in msg.Batch)
            {
                ...
                foreach (var currentFingerPrint in currentRegion.FingerPrints.ToArray())
                {
                    ...
                    var reCalcRootNew = BitConverter.GetBytes(sourceBlock).Keccak256().ToUInt256();
        ...
        private (List<Region>, List<UInt256>, int, int) GetBatch(int[] input, int indexAddressee)
        {
            ...
            foreach (var G in Gs)
            {
                ...
                {
                    hashes.Add(BitConverter.GetBytes(currentInt).Keccak256().ToUInt256());
                }
                ...
                for (var positionIntoSegment = 0; positionIntoSegment < sizeSegment; positionIntoSegment++)
                {
                    ...
                    {
                        Console.WriteLine("NOT CONSIST: Block {0} Root {1} Branch {2}", destinationBlock, destinationRoot, testingBranch);
                    }
        ...
        {
            var tmp = messageValidator.ValidatorIndex;
            var message = new ConsensusMessage
            {
                Validator = new Validator
                {
                    ValidatorIndex = messageValidator.ValidatorIndex,
                    Era = _broadcastId.Era
                },
        ...
        private List<UInt256> recalculateHashes()
        {
            ...
            for (var i = 0; i < numberStripes; i++)
            {
                ...
                for (var j = 0; j < N; j++)
                {
                    hashes.Add(BitConverter.GetBytes(_receivedBatchesOfBlocks[j][i]).Keccak256().ToUInt256());
                }
