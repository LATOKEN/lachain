...
namespace Lachain.Core.Vault
{
    public class KeyGenManager : IKeyGenManager
    {
        ...
        private void BlockManagerOnSystemContractInvoked(object _, InvocationContext context)
        {
            ...
            else if (signature == ContractEncoder.MethodSignatureAsInt(GovernanceInterface.MethodKeygenSendValue))
            {
                ...
                ))
                {
                    ...
                    if (willParticipate)
                    {
                        TransactionReceipt confirmTx;
                        if (HardforkHeights.IsHardfork_15Active(context.Receipt.Block))
                        {
                            confirmTx = MakeConfirmWithOnlyTSTransaction(cycle, keys);
                        }
                        else if (HardforkHeights.IsHardfork_12Active(context.Receipt.Block))
                        {
                            confirmTx = MakeConfirmWithVerificationTransaction(cycle, keys);
                        }
                        else
                        {
                            confirmTx = MakeConfirmTransaction(cycle, keys);
                        }
            ...
            else if (signature == ContractEncoder.MethodSignatureAsInt(GovernanceInterface.MethodKeygenConfirm))
            {
                ...
                if (keygen.HandleConfirm(tpkePublicKey, tsKeys))
                {
                    ...
                    Logger.LogTrace($"Generated keyring with public hash {KeyringHashWithTpkePublicKey(tpkeSharedPublicKey, keys).ToHex()}");
            ...
            {
                Logger.LogDebug($"Detected call of GovernanceContract.{GovernanceInterface.MethodKeygenConfirmWithVerification}");
                var keygen = GetCurrentKeyGen();
                if (keygen is null) return;
                var sender = keygen.GetSenderByPublicKey(context.Receipt.RecoverPublicKey(HardforkHeights.IsHardfork_9Active(context.Receipt.Block)));
                if (sender < 0) 
            ...
            else if (signature == ContractEncoder.MethodSignatureAsInt(GovernanceInterface.MethodKeygenConfirmWithOnlyTS))
            {
                Logger.LogDebug($"Detected call of GovernanceContract.{GovernanceInterface.MethodKeygenConfirmWithOnlyTS}");
                var keygen = GetCurrentKeyGen();
                if (keygen is null) return;
                var sender = keygen.GetSenderByPublicKey(context.Receipt.RecoverPublicKey(HardforkHeights.IsHardfork_9Active(context.Receipt.Block)));
                if (sender < 0) 
                ...
                var args = decoder.Decode(GovernanceInterface.MethodKeygenConfirmWithOnlyTS);
                var cycle = args[0] as UInt256 ?? throw new Exception("Failed to get cycle");
                var tsKeys = new PublicKeySet(
                ...
                if (keygen.HandleConfirm(tsKeys))
                {
                    var keys = keygen.TryGetKeys() ?? throw new Exception();
        ...
        private TransactionReceipt MakeConfirmWithOnlyTSTransaction(UInt256 cycle, ThresholdKeyring keyring)
        {
            Logger.LogTrace("MakeConfirmWithOnlyTSTransaction");
            var tx = _transactionBuilder.InvokeTransactionWithGasPrice(
                _privateWallet.EcdsaKeyPair.PublicKey.GetAddress(),
                ContractRegisterer.GovernanceContract,
                Money.Zero,
                GovernanceInterface.MethodKeygenConfirmWithOnlyTS,
                0,
                cycle,
                keyring.ThresholdSignaturePublicKeySet.Keys.Select(key => key.ToBytes()).ToArray()
            );
            ...
            for (var i = from; i <= to; ++i)
            {
                ...
                foreach (var txHash in block.TransactionHashes)
                {
                    ...
                    else if (signature == ContractEncoder.MethodSignatureAsInt(GovernanceInterface.MethodKeygenConfirmWithOnlyTS))
                    {
                        ...
                        if (!keygen.HandleConfirm(tsKeys)) continue;
                        var keys = keygen.TryGetKeys() ?? throw new Exception();
                        Logger.LogTrace($"Generated keyring with public hash {keys.PublicPartHash().ToHex()}");
                        Logger.LogTrace(
                            $"  - TS public key: {keys.ThresholdSignaturePrivateKey.GetPublicKeyShare().ToHex()}");
                        ...
                        var lastBlockInCurrentCycle = (i / StakingContract.CycleDuration + 1) *
                                                    StakingContract.CycleDuration;
                        _privateWallet.AddThresholdSignatureKeyAfterBlock(
                            lastBlockInCurrentCycle, keys.ThresholdSignaturePrivateKey
                        );
