...
namespace Lachain.Console
{
    class Program
    {
        ...
        {
            if(options.hardforks.ToList().Count != 4) throw new ArgumentException("number of hardfork heights should be 4");
        ...
        private static void UpdateDb(DbOptions options)
        {
            ...
            else if(options.type == "hard")
            {
                ...
                dbShrink.ShrinkDb(
                    options.depth, stateManager.LastApprovedSnapshot.Blocks.GetTotalBlockHeight(), options.consistencyCheck);
                dbContext.CompactAll();
            }
            else 
            {
