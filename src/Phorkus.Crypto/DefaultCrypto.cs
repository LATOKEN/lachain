...
namespace Phorkus.Crypto
{
    public class DefaultCrypto : ICrypto
    {
        ...
        private int RecoveryFromV(byte v)
        {
            ...
            return v - 2 * 41 - 35; // TODO: eth-like chain id?
        }
        ...
        {
            var pk = new byte[64];
            if (!Secp256K1.PublicKeyParse(pk, publicKey)) throw new ArgumentException();
            var messageHash = HashMessage(message);
            var parsedSig = new byte[65];
            var rec = RecoveryFromV(signature[64]);
            if (rec < 0 || rec >= 4) return false;
            return Secp256K1.RecoverableSignatureParseCompact(parsedSig, signature.Take(64).ToArray(), rec) &&
                   Secp256K1.Verify(parsedSig.Take(64).ToArray(), messageHash, pk);
        }
        ...
        public byte[] Sign(byte[] message, byte[] privateKey)
        {
            ...
            if (!Secp256K1.RecoverableSignatureSerializeCompact(serialized, out var rec, sig))
                throw new ArgumentException();

            // TODO: inject chain id to v?
            return serialized.Concat(new[] {(byte) rec}).ToArray();
        }
        ...
        public byte[] RecoverSignature(byte[] message, byte[] signature)
        {
            ...
            var pk = new byte[64];
            var rec = RecoveryFromV(signature[64]);
            if (rec < 0 || rec >= 4) throw new ArgumentException("Bad signature");
            if (!Secp256K1.RecoverableSignatureParseCompact(parsedSig, signature.Take(64).ToArray(), rec))
